...
int readfile(SI *slist)
{
    int i = 0;
    FILE* rfile; 
	rfile = fopen("song.txt","r");     //打开文件，方式为只读，文件名为"song.txt"
    if(!rfile)    //判断是否打开了文件
    {
...
int writefile(SI *slist)
{
    int i;
    FILE* wfile;
	wfile = fopen("song.txt","w");     //打开文件，方式为只写，覆盖式写入
    if(!wfile)     //判断是否打开了文件
    {
        printf("open file error\n");
        return -1;
...
void sort_record()
{
    ...
    for(i = 0; i<x-1; i++)     //冒泡排序
    {
        for(j = 0; j<x-i-1; j++)
        {
            if(strcmp(si[j].pub_date,si[j+1].pub_date)>0)     //按照记录的发行日期进行升序排序
            {
                temp_si = si[j];
                si[j] = si[j+1];
                si[j+1] = temp_si;
